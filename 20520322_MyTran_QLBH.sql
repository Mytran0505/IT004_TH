...
--TRIGGER TRÊN HOADON
DROP TRIGGER TRIGGER_UPDATE_INSERT_HD
CREATE TRIGGER TRIGGER_UPDATE_INSERT_HD	ON HOADON
FOR UPDATE, INSERT 
AS
BEGIN
	DECLARE @NGHD SMALLDATETIME, @NGDK SMALLDATETIME, @MAKH VARCHAR(4)
	...
	IF(@NGHD<@NGDK)
		BEGIN
			PRINT 'ERROR: NGAY MUA HANG KHONG HOP LE'
...
BEGIN
	...
	DECLARE CURSOR_NGHD CURSOR FOR
		SELECT SOHD
		FROM HOADON
		WHERE MAKH=@MAKH
	OPEN CURSOR_NGHD
		FETCH NEXT FROM CURSOR_NGHD INTO @SOHD
		...
		BEGIN
			...
			ELSE
				BEGIN
					FETCH NEXT FROM CURSOR_NGHD
					INTO @SOHD
				END
		END
	CLOSE CURSOR_NGHD
	PRINT 'THANH CONG'
	DEALLOCATE CURSOR_NGHD
END

--12. Ngày bán hàng (NGHD) của một nhân viên phải lớn hơn hoặc bằng ngày nhân viên đó vào làm.

CREATE TRIGGER UPDATE_NV_C12 ON NHANVIEN
FOR UPDATE AS
BEGIN
...
CREATE TRIGGER HD_C12 ON HOADON
...
BEGIN 
	...
	UPDATE	CTHD
	SET	MASP='NONE',SL=0 WHERE SOHD=@SOHD
...
CREATE TRIGGER INSERT_HOADON_C14 ON HOADON
FOR	INSERT AS
BEGIN
	UPDATE	HOADON
	SET	TRIGIA=0
...
CREATE TRIGGER UPDATE_HOADON_C14 ON HOADON
FOR	INSERT AS
BEGIN 
	UPDATE	HOADON
...
CREATE TRIGGER INSERT_CTHD_C14 ON	CTHD
FOR	INSERT AS
BEGIN 
...
CREATE TRIGGER DELETE_CTHD_C14 ON	CTHD
FOR	DELETE AS
BEGIN
	DECLARE	@SL	INT, @GIA MONEY, @SOHD INT
...
CREATE TRIGGER UPDATE_CTHD_C14 ON	CTHD
FOR	UPDATE AS
BEGIN
	...
	IF(@SOHD_CU=@SOHD_MOI) 
		BEGIN 
			UPDATE	HOADON
			SET	TRIGIA=TRIGIA+@SL_MOI*@GIA_MOI-@SL_CU*@GIA_CU WHERE SOHD=@SOHD_CU


			UPDATE	HOADON
			SET	TRIGIA=TRIGIA-@SL_CU*@GIA_CU WHERE SOHD=@SOHD_CU
 
			UPDATE	HOADON
			SET	TRIGIA=TRIGIA+@SL_MOI*@GIA_MOI WHERE SOHD=@SOHD_MOI
		END
...
FOR	INSERT AS
BEGIN
	DECLARE	@GIA_CU	MONEY, @GIA_MOI	MONEY, @SOHD INT, @SL	INT
...
CREATE TRIGGER INSERT_KHACHHANG_C15 ON KHACHHANG
FOR	INSERT AS
BEGIN 
	DECLARE	@MAKH CHAR(4)
	...
	UPDATE	KHACHHANG
	SET	DOANHSO=0 WHERE MAKH=@MAKH

	PRINT 'DA INSERT 1 KHACHHANG MOI VOI DOANHSO BAN DAU LA 0 VND'
END

CREATE TRIGGER UPDATE_KHACHHANG_C15 ON KHACHHANG
FOR	UPDATE AS
BEGIN
	...
	UPDATE	KHACHHANG
	SET	DOANHSO=@DOANHSO_CU WHERE MAKH=@MAKH

	PRINT 'DA UPDATE 1 KHACHHANG'
END

CREATE TRIGGER INSERT_HOADON_C15 ON HOADON
FOR	INSERT AS
BEGIN
...
CREATE TRIGGER DELETE_HOADON_C15 ON HOADON
FOR	DELETE AS
BEGIN
	DECLARE	@TRIGIA	MONEY, @MAKH CHAR(4)
...
CREATE TRIGGER UPDATE_HOADON_C15 ON HOADON
FOR	UPDATE AS
BEGIN
